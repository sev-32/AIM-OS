...
if problematic_imports:
    print(f"⚠️  POTENTIAL ISSUE: Complex imports: {problematic_imports}")

# Check for environment variable dependencies
env_vars = re.findall(r'os\.environ\[[\'"]([^\'"]+)[\'"]\]', content)
if env_vars:
    print(f"⚠️  ENVIRONMENT DEPENDENCIES: {env_vars}")

# Check for API key dependencies
api_keys = re.findall(r'API_KEY', content)
if api_keys:
    print(f"⚠️  API KEY DEPENDENCIES: Found {len(api_keys)} references")
...
if complex_init:
    print(f"⚠️  COMPLEX INITIALIZATION: {complex_init}")

print("\n2. COMPARING WITH WORKING SIMPLE SERVER:")
print("-" * 50)

# Read simple server for comparison
with open("run_mcp_aimos_simple.py", 'r', encoding='utf-8', errors='ignore') as f:
    simple_content = f.read()

simple_imports = re.findall(r'^import\s+(\w+)', simple_content, re.MULTILINE)
print(f"Simple server imports: {simple_imports}")

simple_env_vars = re.findall(r'os\.environ\[[\'"]([^\'"]+)[\'"]\]', simple_content)
print(f"Simple server env vars: {simple_env_vars}")

print("\n3. IDENTIFYING THE BREAKING POINTS:")
print("-" * 50)

# Find where tools are defined
tool_sections = re.findall(r'def handle_tools_call.*?(?=def|\Z)', content, re.DOTALL)
print(f"Tool handling sections: {len(tool_sections)}")
...
print("\n4. TOOLS AFTER THE 6TH (WHAT BREAKS IT?):")
print("-" * 50)
