...
def test_advanced_mcp_server():
    ...
    env["CEREBRASPCA_API_KEY"] = "csk-xv6x26revypveycj6vffvf3yc4fhvx3mxwt9dy6de4xct5ty"
    ...
    try:
        ...
        process.stdin.write(init_request_json)
        process.stdin.flush()
        ...
        if response_line:
            ...
            if "result" in response and "tools" in response["result"]:
                ...
                all_expected = expected_tools + cross_model_tools
                
                print(f"\nExpected tools: {len(all_expected)}")
                print(f"Found tools: {len(tools)}")
                
                missing_tools = [tool for tool in all_expected if tool not in tool_names]
                if missing_tools:
